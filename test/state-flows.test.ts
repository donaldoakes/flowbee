...
describe('state flows', () => {
    ...
    it('data switch state', async () => {
        const translator = new FlowTranslator('test/flows/data-switch-state.flow.yml', options);
        const swfText = fs.readFileSync('test/flows/data-switch-state.flow.yml', 'utf-8');
        const flowbeeText = await translator.toFlowbee(swfText);
        const flowbeeFlow = yaml.load('test/flows/data-switch-state.flow.yml', flowbeeText);
        ...
        // round-trip
        const swf = yaml.load('test/flows/data-switch-state.flow.yml', await translator.toSwf(flowbeeText));
